...
if __name__ == '__main__':
    ...
    print(f"No duplicates rule: {num_valid}/{len(inputs)} passphrases are {color_text('valid', 32)}.")

    words = [list(map(sorted, passphrase.split(' '))) for passphrase in inputs]
    num_valid = sum(map(duplicates, words))
    print(f"No anagrams rule: {num_valid}/{len(inputs)} passphrases are {color_text('valid', 32)}.")
