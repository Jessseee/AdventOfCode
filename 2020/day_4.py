...
def print_passports(valid_passports, invalid_passports, print_all=False):
    ...
    :return:
    """
    print(f"valid passports: {c(len(valid_passports), 32)}")
    print(f"invalid passports: {c(len(invalid_passports), 31)}")
    if print_all:
        print(c("\nVALID PASSPORTS", 32))
        for valid_passport in valid_passports:
            for key in REQUIRED_KEYS:
                print(c(f"{key}: {valid_passport[key]} |", 37), end=" ")
            print("\n")
        print(c("\nINVALID PASSPORTS", 31))
        for invalid_passport in invalid_passports:
            for key in REQUIRED_KEYS:
                if key in invalid_passport:
                    print(c(f"{key}: {invalid_passport[key]} |", 37), end=" ")
            print("\n")
