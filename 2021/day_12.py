...
def find_path(cave, visited, all_paths, visit_one_twice):
    ...
    for connection in all_caves[cave]:
        ...
        can_visit_twice = visit_one_twice \
...
if __name__ == '__main__':
    ...
    print(f"There are {color_text(len(find_path('start', [], [], False)), 32)} distinct paths when only visiting small caves once.")
    ...
    print(f"There are {color_text(len(find_path('start', [], [], True)), 32)} distinct paths when visiting one small cave twice.")
