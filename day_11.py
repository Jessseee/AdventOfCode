...
if __name__ == '__main__':
    ...
    grid = import_input().read()\
        .replace('.', str(GRID_CELL['floor']))\
        .replace('L', str(GRID_CELL['unoccupied']))\
        .split('\n')
    grid = [[int(char) for char in row] for row in grid]
    grid_size = [len(grid), len(grid[0])]
    print(f'Tolerance 4 and only check adjacent seats\n'
          f'Total occupied seats:',
          sum([row.count(GRID_CELL["occupied"])
               for row in get_stable_seating(grid, check_adjacent, 4)]))
    print(f'Tolerance 5 and check nearest visible seats\n'
          f'Total occupied seats:',
          sum([row.count(GRID_CELL["occupied"])
               for row in get_stable_seating(grid, check_visible, 5)]))
