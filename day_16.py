...
def scan_tickets(tickets, rules):
    ...
    for ticket in tickets:
        for ticket_value in ticket:
            eval_ticket_value = [not (r[0] <= ticket_value <= r[1] or r[2] <= ticket_value <= r[3]) for r in rules.values()]
...
def check_field_position(tickets, rules):
    ...
    for i in range(len(rules)):
        ...
        for name, r in rules.items():
            eval_ticket_values = [r[0] <= ticket_field_i <= r[1] or r[2] <= ticket_field_i <= r[3] for ticket_field_i in tickets_field_i]
